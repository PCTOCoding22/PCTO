...
int aggiungi_numero(int numero)
{ 
    numero = numero + 1;
    return(numero);
}
...
int main ()
{
    ...
    risultato_aumentato = aggiungi_numero(numero);   //gli dico di applicare la funzione +1 per ottenere risultato_aumentato

 printf("il mio numero è: %d\n",risultato_aumentato);
                                                      //finisce la prima parte dove aumenta il numero
 int contatore =0;
  while(contatore < risultato_aumentato)  //dichiaro che la variabile contatore ha valore< di risultato aumentato
  { 
